...
public class GetTeamsTasksTask extends AsyncTask<String, Void, JSONObject> {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        HttpPost getTeamsTasksRequest = new HttpPost(url);
        StringEntity stringEntity = new StringEntity(createRequestData(token, teamName, username),
                "UTF-8");
        stringEntity.setContentType("application/json");
        getTeamsTasksRequest.setEntity(stringEntity);
        try {
            HttpResponse response = client.execute(getTeamsTasksRequest);
