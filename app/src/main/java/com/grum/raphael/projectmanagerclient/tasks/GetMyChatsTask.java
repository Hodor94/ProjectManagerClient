...
public class GetMyChatsTask extends AsyncTask<String, Void, JSONObject> {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        HttpPost getMyChatsRequest = new HttpPost(url);
        StringEntity stringEntity = new StringEntity(createRequestData(token, username, teamName),
                "UTF-8");
        stringEntity.setContentType("application/json");
        getMyChatsRequest.setEntity(stringEntity);
        try {
            HttpResponse response = client.execute(getMyChatsRequest);
