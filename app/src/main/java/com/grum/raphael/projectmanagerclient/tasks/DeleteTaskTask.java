...
public class DeleteTaskTask extends AsyncTask<String, Void, JSONObject> {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        HttpPost deleteTaskRequest = new HttpPost(url);
        StringEntity stringEntity = new StringEntity(createRequestData(token, taskName, teamName,
                username), "UTF-8");
        stringEntity.setContentType("application/json");
        deleteTaskRequest.setEntity(stringEntity);
        try {
            HttpResponse response = client.execute(deleteTaskRequest);
