...
public class GetMyTasksTask extends AsyncTask<String, Void, JSONObject> {
    @Override
    protected JSONObject doInBackground(String... params) {
        ...
        HttpPost getMyTasksRequest = new HttpPost(url);
        StringEntity stringEntity = new StringEntity(createRequestData(token, username), "UTF-8");
        stringEntity.setContentType("application/json");
        getMyTasksRequest.setEntity(stringEntity);
        try {
            HttpResponse response = client.execute(getMyTasksRequest);
