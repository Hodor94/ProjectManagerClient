...
public class GetMyProjectsTask extends AsyncTask<String, Void, JSONObject> {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        HttpPost getMyProjectsRequest = new HttpPost(url);
        StringEntity stringEntity = new StringEntity(createRequestData(token, username, teamName),
                "UTF-8");
        stringEntity.setContentType("application/json");
        getMyProjectsRequest.setEntity(stringEntity);
        try {
            HttpResponse response = client.execute(getMyProjectsRequest);
