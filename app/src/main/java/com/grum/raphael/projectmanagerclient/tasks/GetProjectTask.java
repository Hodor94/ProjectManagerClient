...
public class GetProjectTask extends AsyncTask<String, Void, JSONObject> {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        HttpPost getProjectRequest = new HttpPost(url);
        StringEntity stringEntity = new StringEntity(createRequestData(token, projectName, teamName),
                "UTF-8");
        stringEntity.setContentType("application/json");
        getProjectRequest.setEntity(stringEntity);
        try {
            HttpResponse response = client.execute(getProjectRequest);
