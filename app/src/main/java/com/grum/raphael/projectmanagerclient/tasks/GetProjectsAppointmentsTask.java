...
public class GetProjectsAppointmentsTask extends AsyncTask<String, Void, JSONObject> {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        HttpPost getProjectsAppointmentsRequest = new HttpPost(url);
        StringEntity stringEntity = new StringEntity(createRequestData(token, projectName, teamName),
                "UTF-8");
        stringEntity.setContentType("application/json");
        getProjectsAppointmentsRequest.setEntity(stringEntity);
        try {
            HttpResponse response = client.execute(getProjectsAppointmentsRequest);
