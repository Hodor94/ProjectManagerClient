...
public class GetProjectMembersTask extends AsyncTask<String, Void, JSONObject> {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        HttpPost getProjectMembersRequest = new HttpPost(url);
        StringEntity stringEntity = new StringEntity(createRequestData(token, projectName,
                teamName, username), "UTF-8");
        stringEntity.setContentType("application/json");
        getProjectMembersRequest.setEntity(stringEntity);
        try {
            HttpResponse response = client.execute(getProjectMembersRequest);
