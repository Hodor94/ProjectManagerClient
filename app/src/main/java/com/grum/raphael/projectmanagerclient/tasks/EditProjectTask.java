...
public class EditProjectTask extends AsyncTask<String, Void, JSONObject> {
    @Override
    protected JSONObject doInBackground(String... params) {
        ...
        HttpPost editProjectRequest = new HttpPost(url);
        StringEntity stringEntity = new StringEntity(createRequestData(token, username, projectName,
                teamName, description, deadline), "UTF-8");
        stringEntity.setContentType("application/json");
        editProjectRequest.setEntity(stringEntity);
        try {
            HttpResponse response = client.execute(editProjectRequest);
    ...
    private String createRequestData(String token, String username, String projectName,
                                     String teamName, String description, String deadline) {
        return "{\"token\": \"" + token + "\", \"username\": \"" + username + "\", \"projectName\":"
                + " \"" + projectName + "\", \"teamName\": \"" + teamName + "\", \"description\": "
